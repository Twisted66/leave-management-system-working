...
export const createAbsenceRecord = api({ expose: true, method: "POST", path: "/absence-records", auth: true }, async (req) => {
    ...
    const absenceRecord = await leaveDB.queryRow `
...
export const listAbsenceRecords = api({ expose: true, method: "GET", path: "/absence-records", auth: true }, async (params) => {
    const auth = getAuthData();
    ...
    `;
    const records = await leaveDB.rawQueryAll(query, ...queryParams);
    return { records };
});
// Creates a new absence conversion request.
export const createAbsenceConversionRequest = api({ expose: true, method: "POST", path: "/absence-conversion-requests", auth: true }, async (req) => {
    const auth = getAuthData();
    ...
    const conversionRequest = await leaveDB.queryRow `
...
export const listAbsenceConversionRequests = api({ expose: true, method: "GET", path: "/absence-conversion-requests", auth: true }, async (params) => {
    const auth = getAuthData();
    let whereClause = "WHERE 1=1";
    const queryParams = [];
    // Apply role-based filtering
...
export const updateAbsenceConversionStatus = api({ expose: true, method: "PUT", path: "/absence-conversion-requests/:id/status", auth: true }, async (req) => {
    const auth = getAuthData();
    ...
    if (req.status === 'approved') {
        ...
        const annualLeaveType = await leaveDB.queryRow `
...
//# sourceMappingURL=absence_management.js.map