...
export const sendExpiryNotification = api({ expose: true, method: "POST", path: "/notifications/document-expiry", auth: true }, async (req) => {
...
export const checkExpiringDocuments = api({ expose: true, method: "POST", path: "/notifications/check-expiring", auth: true }, async () => {
    const auth = getAuthData();
    ...
    const hrEmails = await leaveDB.queryAll `
